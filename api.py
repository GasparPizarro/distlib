...
class Books(MethodView):
    ...
    def get(self, book_id=None):
        if not book_id:
            ...
            book_count = int(
                query_db(
                    "select count(*) from book where owner = ?", (g.user,), one=True
                )[0]
            )
            books = query_db(
                ...
                [
                    {
                        ...
                        "bearer": (
                            query_db(
                                "select recipient from loan where book = ? and status = 2",
                                (id,),
                                one=True,
                            )[0]
                            if query_db(
            ...
            try:
                id_, owner, title, author, year = query_db(
                    "select id, owner, title, author, year from book where id = ?",
                    (book_id,),
                    one=True,
                )
            except TypeError:
...
def book_search():
    ...
    if mine:
        ...
        book_count = int(
            query_db(
                "select count(*) from book where (title like ? or author like ?) and owner != ?",
                ("%" + query + "%", "%" + query + "%", g.user),
                one=True,
            )[0]
        )
        books = query_db(
            "select id, owner, title, author, year from book where (title like ? or author like ?) and owner != ? limit ? offset ?",
            ("%" + query + "%", "%" + query + "%", g.user, size, (page - 1) * size),
        )
...
def debts():
    debts = query_db(
        "select id, book, start, span, status from loan where recipient = ? and status = 2",
        (g.user,),
    )
    return jsonify(
        [
            {
                ...
                "book": {
                    ...
                    for (field, value) in zip(
                        ...
                        query_db(
                            "select * from book where id = ?", (debt[1],), one=True
                        ),
                    )
                },
                "lender": query_db(
                    "select owner from book where id = ?", (debt[1],), one=True
                )[0],
                "start": debt[2],
                "span": debt[3],
                "status": debt[4],
...
class Loans(MethodView):
    ...
    def get(self, loan_id=None):
        if loan_id:
            loan = query_db(
                "select id, book, recipient, start, span, status from loan where id = ?",
                (loan_id,),
                one=True,
            )
            if loan:
            ...
            loans = query_db(
                ...
                "where book in (select id from book where owner = ?) and status in (0, 2)",
                (g.user,),
            )
            return jsonify(
                [
    ...
    def patch(self, loan_id):
        ...
        if status == "accepted":
            (owner,) = query_db(
                "select owner from book where id = (select book from loan where id = ?)",
                (loan_id,),
                one=True,
            )
            if owner != g.user:
            ...
            query_db(
                "update loan set status = ?, start = ? where id = ?",
                (1, datetime.datetime.today(), loan_id),
            )
            id, book, recipient, start, span, status = query_db(
                "select id, book, recipient, start, span, status from loan where id = ?",
                (loan_id,),
            ...
            query_db(
                "update loan set status = ?, start = ? where id = ?",
                (3, datetime.datetime.today(), loan_id),
            )
            id, book, recipient, start, span, status = query_db(
                "select id, book, recipient, start, span, status from loan where id = ?",
                (loan_id,),
...
class Profile(MethodView):
    ...
    def get(self):
        profile = query_db(
            "select username, first_name, last_name from user where username = ?",
            (g.user,),
            one=True,
        )
    ...
    def post(self):
        ...
        if new_password:
            hashed_password = query_db(
                "select password from user where username = ?", (g.user,), one=True
            )
            if not (
                old_password
                and bcrypt.checkpw(
                    old_password.encode("utf-8"), hashed_password[0].encode("utf-8")
                )
            ):
        ...
        if last_name:
            query_db(
                "update user set last_name = ? where username = ?", (last_name, g.user)
            )
        profile = query_db(
            "select username, first_name, last_name from user where username = ?",
            (g.user,),
...
def get_token():
    ...
    hashed_password = query_db(
        "select password from user where username = ?", (username,), one=True
    )
    if hashed_password is not None and bcrypt.checkpw(
        password.encode("utf-8"), hashed_password[0].encode("utf-8")
    ):
        token = query_db(
            "select token from user_token where username = ?", (username,), one=True
        )
        if token is None:
            token = "".join(
...
app.add_url_rule(
    "/books/<int:book_id>", view_func=books_view, methods=["GET", "PUT", "DELETE"]
)
